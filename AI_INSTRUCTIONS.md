...
## Runnable portfolio / capability routing

Before reporting IDLE, evaluate the safe admitted non-overlapping portfolio after higher-priority security, MAIN_RED, live-ownership, review, and integration gates. CLAIM conflict, dependency/review wait, or a local capability mismatch does not by itself justify IDLE.

For work that names a shared or specialized executor, a local capability mismatch is a routing condition rather than a global blocker. Attempt or freshly discover the admitted capable executor and preserve evidence of that attempt. Continue searching other admitted non-overlapping work while routing is pending. Workers must not invent implementation workstreams; new implementation still requires #16 admission.

IDLE is valid only when the portfolio contains neither locally runnable work nor work routable to an admitted capable executor, or when an explicit safety/Human Required stop applies. Deterministic enforcement lives in `scripts/runnable_portfolio.py`.
